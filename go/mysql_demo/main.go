...
// 初始化数据库连接池
func initDb(addr, port, user, passworld, db_name string) (err error) {
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", user, passworld, addr, port, db_name)
...
func queryOne() {
	...
	rowObj := DB.QueryRow(sqlStr, 1)	// 查询行
...
// 插入数据(DB.Exec)
func insert() {
	sqlStr := `insert into user(name, age) value("张渊", 5)`
...
func main() {
	err := initDb("127.0.0.1", "3306", "root", "123456", "go_test")
	if err != nil {
